          ...
          BEGIN
            ...
            INSERT INTO sync_ddmast
              ...
              (SELECT /*+ ALL_ROWS */
                ...
                a.rate,
                a.accrue
               FROM   svdatpv51.ddmast@dblink_data a
          ...
          BEGIN
            ...
            INSERT INTO sync_cdmast
              ...
              (SELECT /* +ALL_ROWS */
                ...
                b.renew,
                b.dactn
               FROM   svdatpv51.cdmast@dblink_data b
               WHERE
        ...
        BEGIN
            ...
            THEN
                UPDATE SET
                    ...
                    c.status = a.status,
                    c.account_no = LPAD (a.cdnum, 14, '0')
            WHEN NOT MATCHED
            THEN
                INSERT              (c.receipt_no,
                                     c.product_code,
          ...
          BEGIN
            ...
            INSERT INTO sync_cfagrp a
              ...
              (SELECT b.cfcifn,
                      ...
                      b.cfagty,
                      b.cfagd7
               FROM   svdatpv51.cfagrp@dblink_data b
        ...
        BEGIN
            ...
            THEN
                UPDATE SET
                    ...
                    c.branch_no = SUBSTR (LPAD (src.cfagno, 14, 0), 0, 3)
            WHEN NOT MATCHED
            THEN
                INSERT              (c.acct_no,
                                     c.acct_name,
                                     ...
                                     c.update_time,
                                     c.bank_no)
                    VALUES   (LPAD (src.cfagno, 14, '0'),
                              trim(src.cfgnam),
                              src.cfcifn,
                              'FD',
                              TRIM (src.cfgcur),
                              SUBSTR (LPAD (src.cfagno, 14, 0), 0, 3),
                              --tam thoi
                              SUBSTR (LPAD (src.cfagno, 14, 0), 0, 3),
                              DECODE (LENGTH (src.cfagd7),
                                      7, TO_DATE (src.cfagd7, 'yyyyddd'),
    ...
    WHEN MATCHED THEN
      ...
      SET    c.acct_name     = trim(src.cfgnam),
             ...
             c.status        = DECODE(TRIM(src.cfgsts),
                                      ...
                                      'CLOS'),
             c.bank_no       = '302', --tam thoi
             c.branch_no     = SUBSTR(LPAD(src.cfagno,
                                           14,
                                           0),
                                      0,
                                      3)
          ...
          BEGIN
            ...
            INSERT INTO sync_lnmast
              SELECT /*+ ALL_ROWS */
               ...
               b.rate,
               trim(b.tmcode)
              FROM   svdatpv51.lnmast@dblink_data b
              --WHERE  b.status <> 2
              WHERE  /*b.curtyp = 'VND'
                                                                             AND*/
    ...
    LOOP
        BEGIN
            ...
            THEN
                UPDATE SET
                    c.bank_no = '302',                            --a.bkn,
                    c.org_no = LPAD (a.brn, 3, '0'),              --a.brn,
                    c.branch_no = LPAD (a.brn, 3, '0'),           --a.brn,
                    c.accured_interest = a.accint,
                    --c.full_release_date?
          ...
          BEGIN
            ...
            INSERT INTO sync_cdtnew
              SELECT /*+ ALL_ROWS */
               ...
               b.dactn,
               b.cdterm
              FROM   svdatpv51.cdtnew@dblink_data b
...
--                   COMMIT;
    ...
    FORALL i IN acct_bkr_rpno_len_tab.FIRST .. acct_bkr_rpno_len_tab.LAST
        UPDATE   bk_receipt_info a
           SET   a.receipt_no = LPAD (a.receipt_no, 14, '0')
    ...
    FORALL i IN acct_bkr_acno_len_tab.FIRST .. acct_bkr_acno_len_tab.LAST
        UPDATE   bk_receipt_info a
           SET   a.account_no = LPAD (a.account_no, 14, '0')
          ...
          BEGIN
            ...
            INSERT INTO sync_lntnew
              SELECT /*+ ALL_ROWS */
               ...
               b.rate,
               trim(b.tmcode)
              FROM   svdatpv51.lntnew@dblink_data b
              WHERE  b.status <> 2
                    /*AND b.curtyp = 'VND'*/
              AND    b.cifno BETWEEN g_min_count AND g_max_count;
            ...
            INSERT INTO sync_ddmemo
              ...
              (SELECT /*+ ALL_ROWS */
               ...
               --a.branch,
                ...
                --TRIM(a.sccode),
                TRIM(a.acname)
               FROM   svdatpv51.ddmemo@dblink_data a
               /*WHERE  LPAD(a.acctno,
               14,
               '0') IN (SELECT acct_no
      ...
      BEGIN
        ...
        INSERT INTO sync_cdmemo
          ...
          (SELECT /*+ ALL_ROWS */
            ...
            b.cdnum,
            b.status
           FROM   svdatpv51.cdmemo@dblink_data b /*WHERE  LPAD(b.acctno,
                                                                                              14,
                                                                                              '0') IN (SELECT acct_no
                                                                                                       FROM   sync_account_info)*/
           );
      ...
      BEGIN
        ...
        INSERT INTO sync_lnmemo
          SELECT /*+ ALL_ROWS */
           ...
           b.comacc,
           b.othchg
          FROM   svdatpv51.lnmemo@dblink_data b /*WHERE  LPAD(b.acctno,
                                                                                          14,
                                                                                          '0') IN (SELECT acct_no
                                                                                                   FROM   sync_account_info)*/
          ;
  ...
  insert into DDMASTER (select ACCTNO,actype,cbal from svdatpv51.ddmast@dblink_data where status != 2);
  insert into DDMASTER (select ACCTNO,actype,cbal from svdatpv51.ddtnew@dblink_data where status != 2);
  END;

  PROCEDURE proc_cdmaster IS
  BEGIN
      execute immediate 'truncate table CDMASTER';
  insert into CDMASTER (select ACCTNO,actype,cbal from svdatpv51.cdmast@dblink_data where status != 2);
  insert into CDMASTER (select ACCTNO,actype,cbal from svdatpv51.cdtnew@dblink_data where status != 2);
  END;

    PROCEDURE proc_lnmaster IS
  BEGIN
  execute immediate 'truncate table LNMASTER';
  insert into LNMASTER (select ACCTNO,actype,cbal from svdatpv51.lnmast@dblink_data where status != 2);
  insert into LNMASTER (select ACCTNO,actype,cbal from svdatpv51.lntnew@dblink_data where status != 2);
  END;
