...
--        COMMIT;
        ...
        ON (c.tran_sn = src.tran_sn AND c.rollout_acct_no = LPAD(src.tmacctno, 14, '0') AND c.amount = src.tmtxamt)
        WHEN MATCHED THEN
          UPDATE
          SET    c.status  = DECODE(src.tmtxstat,
                                    'CE',
          ...
          BEGIN
            ...
            INSERT INTO sync_ddtrn2
              SELECT a.trancd,
                     ...
                     a.treffd,
                     a.trtime
              FROM   svdatpv51.ddtrn2@dblink_data a
              WHERE  a.dorc IN ('D',
                                'C')
          ...
          BEGIN
            ...
            INSERT INTO sync_lnhist
              SELECT /*+ ALL_ROWS */
               ...
               a.lhosbl, --anhnt6
               a.lhdudt -- anhnt6
              FROM   svhispv51.lnhist@dblink_data a
              WHERE  a.lhdorc IN ('D',
                                  'C')
