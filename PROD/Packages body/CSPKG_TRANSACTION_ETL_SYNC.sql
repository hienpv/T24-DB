--------------------------------------------------------
--  DDL for Package Body CSPKG_TRANSACTION_ETL_SYNC
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "IBS"."CSPKG_TRANSACTION_ETL_SYNC" 
IS
 /*----------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------*/
    ...
    BEGIN
      ...
      DBMS_STATS.gather_table_stats (
        ...
        IF l_count = 0 THEN
            ...
            WHERE  sync_type IN (1, 2, 3, 5, 6, 7, 8)
          --AND    TRUNC(tran_time) = l_etl_date);--TRUNC(SYSDATE - 1); --1:dd transaction comment an index 09/10/2018
            AND   tran_time >= l_etl_date 
            AND   Tran_Time < l_etl_date + 1);
    ...
    -- 2019-12-18 thinhdd: This delete runs too slow, 180krows> 50mins - causing locks, change to hint
        ...
        WHERE  sync_type IN (1, 2, 3, 5, 6, 7, 8)
       -- AND    TRUNC(tran_time) = l_etl_date;--TRUNC(SYSDATE - 1); comment an index 09/10/2018
        AND   tran_time >= l_etl_date 
        AND   Tran_Time < l_etl_date + 1 ;
        ...
        LOOP
            ...
            INSERT INTO sync_lnhist
              SELECT
               ...
               a.lhosbl,
               a.lhdudt
              FROM   --staging.si_his_lnhist@staging_pro a,
                    svhispv51.lnhist@dblink_data a, --20160426 QuanPD changed STAGING->CORE
                    sync_account_info x
        ...
        BEGIN
            ...
            INSERT INTO twtb_account_history (core_sn, --20160301 QuanPD dung bang tam roi moi day vao bang chinh
                ...
                (SELECT
                          ...
                          DECODE(a.trancd,'160','Tra lai TK KKH',SUBSTR (a.trefth, 11, LENGTH (a.trefth))),
                          LPAD (a.tracct, 14, '0'),
                          SYSDATE,
                          6,
                          'SUCC',
    ...
    INSERT INTO sync_account_info a
      (SELECT bai.cif_no,
       ...
       WHERE  bai.cif_no IN (SELECT cif_no
                             FROM   sync_cif_n sct)
       AND    bai.acct_type IN ('CA','SA','31'));

    --Get  data from DDHIST core
    g_error_level := 3;
  --- 20210929 QuanLD Turning
    LOOP
      BEGIN
        g_min_time := g_max_time;
        g_max_time := g_min_time + g_limit_time;
        ...
        INSERT INTO sync_ddhist
          SELECT 
           ...
           TRIM(a.truser),
           TRIM(a.auxtrc)
          FROM   --staging.si_his_ddhist@STAGING_PRO a,
                svhispv51.ddhist@dblink_data a, --20160426 QuanPD changed STAGING->CORE 
