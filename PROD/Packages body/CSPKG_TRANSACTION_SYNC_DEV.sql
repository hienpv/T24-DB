--------------------------------------------------------
--  DDL for Package Body CSPKG_TRANSACTION_SYNC_DEV
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "IBS"."CSPKG_TRANSACTION_SYNC_DEV" 
IS
 /*----------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------*/
    ...
    BEGIN
        IF dm_operation_type = 'I' AND orchestrate_dorc IN ('D','C') THEN
            l_acc_no := LPAD (orchestrate_tracct, 14, '0');
...
--                    AND c.amount = orchestrate_tmtxamt;
    ...
    BEGIN
        ...
        THEN
          ...
          --  dbms_output.put_line ('TROI OI LA TROI 1 l_check_exists = ' || l_check_exists);
            ...
            IF l_check_exists = 1 AND l_tmtran_fail = 'N' THEN --haons add 20150410
                ...
                EXCEPTION--#20150425 Loctx add check exception
                    WHEN NO_DATA_FOUND THEN
                        NULL;
                END;
                  --  dbms_output.put_line ('TROI OI LA TROI 1');
                    l_taget_table := 'BK_ACCOUNT_HISTORY';
