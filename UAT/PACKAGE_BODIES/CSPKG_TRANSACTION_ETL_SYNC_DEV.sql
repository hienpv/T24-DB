...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "T24IBS"."CSPKG_TRANSACTION_ETL_SYNC_DEV" 
IS
 /*----------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------*/
   ...
   BEGIN
        ...
        LOOP
            ...
            INSERT INTO sync_lnhist
              SELECT
               ...
               a.lhosbl,
               a.lhdudt
              FROM   STAGINGUAT.SI_HIS_LNHIST@STAGING_PRO_CORE a,sync_account_info x
        ...
        EXECUTE IMMEDIATE 'alter session close database link staging_pro_core';
          ...
          BEGIN
            ...
            INSERT INTO sync_cdhist
              SELECT
               ...
               TRIM(a.chuser),
               a.chtime
              FROM   STAGINGUAT.si_his_cdhist@staging_pro_core a,sync_account_info b
      ...
      BEGIN
        ...
        INSERT INTO sync_ddhist
          SELECT
           ...
           TRIM(a.truser),
           TRIM(a.auxtrc)
          FROM   STAGINGUAT.si_his_ddhist@STAGING_PRO_core a,sync_account_info x
          WHERE  a.dorc IN ('D','C')
