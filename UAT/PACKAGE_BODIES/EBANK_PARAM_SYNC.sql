...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "T24IBS"."EBANK_PARAM_SYNC" IS
  v_start_date DATE;
...
--      FROM   SVPARPV51.ssfxrtl1@DBLINK_DATA;
  ...
  BEGIN
    ...
    WHEN MATCHED THEN
      UPDATE
      SET    a.branch_code      = (case when length(src.ssobrn) <= 2 then LPAD (src.ssobrn, 3, '0') else TO_CHAR(src.ssobrn) end),
    ...
    WHEN NOT MATCHED THEN
      ...
      VALUES
        (src.ssooff,
        (case when length(src.ssobrn) <= 2 then LPAD (src.ssobrn, 3, '0') else TO_CHAR(src.ssobrn) end),
         src.ssoidn,
         src.ssonam,
         src.ssosna,
         src.ssocur,
