...
----------------------------------------------------------------------------------------------------*/
    ...
    BEGIN
        ...
        WHERE a.TRAN_TIME <= TO_NUMBER(TO_CHAR(TO_DATE(p_etl_date, 'RRRRDDD') - 2, 'RRRRDDD'));
...
*/
        ...
        WHERE a.trans_date <= l_etl_date - 7;
        ...
        WHERE RMEND7 < TO_NUMBER(TO_CHAR(TO_DATE(p_etl_date, 'RRRRDDD') - 30, 'RRRRDDD'));
        COMMIT;

        DELETE FROM sync_cdc_rmdetl--#20150418 Loctx: disable action xoa cua CDC va thuc hien xoa thu cong
        WHERE RDJEFF < TO_NUMBER(TO_CHAR(TO_DATE(p_etl_date, 'RRRRDDD') - 30, 'RRRRDDD'));
        COMMIT;
        ...
        IF l_check_exists > 0 THEN
            ...
            INSERT INTO t24ibs.cstb_data_later_process_hist   -- ibs.
            (
