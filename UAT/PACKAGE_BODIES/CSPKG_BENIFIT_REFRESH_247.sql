...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "T24IBS"."CSPKG_BENIFIT_REFRESH_247" AS
	
    PROCEDURE pr_benifit_internal_txn
    IS
        CURSOR c_txn (p_batch_no NUMBER)
        IS
                SELECT h.row_id h_rowid, (case when length(tm.rollout_acct_no) = 13 then LPAD (tm.rollout_acct_no, 14, '0') else TO_CHAR(tm.rollout_acct_no) end) beneficiary_acct_no,
