...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "T24IBS"."PK_PROCESS_DATA" AS
...
--    commit;
  ...
  BEGIN
    ...
    update bb_user_info set login_pwd='96e79218965eb72c92a549dd5a330112' where login_pwd !='96e79218965eb72c92a549dd5a330112';
    commit;
  END;
...
--      commit;
  ...
  BEGIN
    ...
    FOR user_rec IN (
      ...
      left join STG.SI_DAT_CFMAST@STAGING_PRO d on to_number(c.cif_no) = trim(d.CFCIFN)
      where c.cif_no is not null and d.CFCIFN is not null
    )
    LOOP
      insert into BK_CIF (CIF_NO, CERT_TYPE, CERT_CODE, BANK_NO, ORG_NO, CIF_ACCT_NAME, BIRTH_DATE, BIRTH_PLACE, COUNTRY, INDIVIDUAL,
