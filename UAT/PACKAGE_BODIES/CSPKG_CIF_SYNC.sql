...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "T24IBS"."CSPKG_CIF_SYNC" 
IS
 /*----------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------*/
    ...
    CSPKS_CDC_T24_UTIL.PR_T24_CDD_LOG_CFMAST ( dm_operation_type , orchestrate_cfcifn ,  orchestrate_cfsscd ,  orchestrate_cfssno , 
                                  orchestrate_cfbrnn ,  orchestrate_cfna1  ,   orchestrate_cfbird ,  orchestrate_cfbirp ,  orchestrate_cfcitz , 
                                  orchestrate_cfindi ,  orchestrate_taxcod , 0 , 0 , 'CDD INPUT' ) ;
        IF dm_operation_type <> 'D' AND LENGTH(RTRIM(orchestrate_cfssno)) < 40 THEN
            ...
            USING (SELECT
                    ...
                    (case when length(TRIM(orchestrate_cfbrnn)) <= 2 then LPAD (TRIM(orchestrate_cfbrnn), 3, '0') else TO_CHAR(TRIM(orchestrate_cfbrnn)) end) AS cfbrnn,
    ...
    EXCEPTION
        WHEN OTHERS THEN
           ...
           CSPKS_CDC_T24_UTIL.PR_T24_CDD_LOG_CFMAST ( dm_operation_type , orchestrate_cfcifn ,  orchestrate_cfsscd ,  orchestrate_cfssno , 
                                  orchestrate_cfbrnn ,  orchestrate_cfna1  ,   orchestrate_cfbird ,  orchestrate_cfbirp ,  orchestrate_cfcitz , 
                                  orchestrate_cfindi ,  orchestrate_taxcod , 0 , -1 , l_err_desc ) ;
            COMMIT;
            RAISE;
