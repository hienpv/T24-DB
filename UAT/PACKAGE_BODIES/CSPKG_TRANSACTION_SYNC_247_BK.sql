--------------------------------------------------------
--  DDL for Package Body CSPKG_TRANSACTION_SYNC_247_BK
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "T24IBS"."CSPKG_TRANSACTION_SYNC_247_BK" AS
 ...
 PROCEDURE  pr_tmtran_fail_sync_247 (dm_operation_type in CHAR,
    ...
    IS
        ...
        l_rowid ROWID;
    BEGIN
         cspks_cdc_util.pr_log_sync_error('SIBS', 'TTR24', 'BK_ACCOUNT_HISTORY',
        ...
        THEN
            ...
            IF 1 = 1 THEN        --CHIPM bo phan check dieu kien ton tai CIF    
    ...
    BEGIN
      ...
      cspks_cdc_util.pr_log_sync_error('SIBS', 'TTR24', 'BK_ACCOUNT_HISTORY',
        ...
        THEN
            ...
            IF l_tmtran_fail = 'N' THEN  -- --CHIPM bo phan check dieu kien ton tai CIF   
                --#20150804 Loctx add
                BEGIN
                    SELECT MAX(tran_sn), MAX(sender_id)
                    INTO l_tran_sn, l_sender_id
                ...
                END;
                    ...
                    SELECT
                             TRIM(orchestrate_tmresv07) || seq_core_sn.NEXTVAL,
                                TO_DATE( ( to_number(to_char(sysdate, 'yyyyddd')) || ':' ||
                                        LPAD(orchestrate_tmtiment,
                                              6,
                                              '0')),
                                        'yyyyddd:hh24miss') AS tran_date,
                    ...
                    INSERT INTO bk_account_history_temp c
                            (c.AMOUNT,
                             c.TRAN_TIME,
                             c.TELLER_ID,
                             c.TM_SEQ)
