...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "T24IBS"."CSPKG_ACCOUNT_ETL_SYNC_DEV_MDB" 
IS
/*----------------------------------------------------------------------------------------------------
                     ...
                     USING   (SELECT b.bkn,
                               ...
                               WHERE   b.acctno <> 0
                                       ...
                                       AND NOT EXISTS (SELECT * FROM ibs.cstb_branch_mdb_loai_tk mdb
                                                        WHERE (case when length(mdb.branch_code) <= 2 then LPAD (mdb.branch_code, 3, '0') else TO_CHAR(mdb.branch_code) end) = b.brn )
                             ) a
