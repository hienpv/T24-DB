...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "T24IBS"."PK_ACTIVITI_PROCESS" AS
  PROCEDURE PROCESS_DATA_ACTIVITI
  AS
  BEGIN
    FOR activiti_rec IN (
      select * from ACT_HI_PROCINST where start_time_ < sysdate - 8  and rownum <= 1000 order by start_time_      
    )
    LOOP
