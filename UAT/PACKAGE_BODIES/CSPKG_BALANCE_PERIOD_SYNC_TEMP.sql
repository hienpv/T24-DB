  ...
  ----------------------------------------------------------------------------------------------------*/
    ...
    BEGIN
          ...
          USING
                ...
                WHERE   bka.acct_no = (case when length(memo.acctno) = 13 then LPAD (memo.acctno, 14, '0') else TO_CHAR(memo.acctno) end) 
    ...
    BEGIN
          ...
          USING
                ...
                FROM    ibs.bk_account_info bka,
                        ...
                        RAWSTAGEUAT.SI_DAT_CDMEMO@RAWSTAGE_PRO_CORE memo --#20160611 Loctx change tam thoi gi RAWSTAGE LOI JOB RWSIDD
                WHERE   bka.acct_no = (case when length(memo.acctno) = 13 then LPAD (memo.acctno, 14, '0') else TO_CHAR(memo.acctno) end)
