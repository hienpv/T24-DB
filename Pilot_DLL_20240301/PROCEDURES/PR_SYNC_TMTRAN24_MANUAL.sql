--------------------------------------------------------
--  DDL for Procedure PR_SYNC_TMTRAN24_MANUAL
--------------------------------------------------------
set define off;
...
begin
    ...
    FROM svdatp24h.tmtr24@dblink_data
        WHERE tmapptype <> 'G'
    ...
    LOOP
        ...
        LOOP
            IF l_txn_list(i).IORU_IND = 'I' THEN
                ibs.cspkg_transaction_sync_247.pr_tmtran_sync ('I',
                                    l_txn_list(i).tmtxcd,
            ...
            ELSE
                ...
                ibs.cspkg_transaction_sync_247.pr_tmtran_fail_sync ('U',
                                   l_txn_list(i).tmtxcd,
                                    l_txn_list(i).tmresv07,
                                    l_txn_list(i).tmdorc,
                                    l_txn_list(i).tmtxamt,
