--------------------------------------------------------
--  DDL for Package Body CSPKG_TRANSACTION_ETL_SYNC_DEV
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "IBS"."CSPKG_TRANSACTION_ETL_SYNC_DEV" 
IS
 /*----------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------*/
    ...
    BEGIN
        ...
        DELETE FROM sync_bk_account_history a --MOVE FROM proc_del_tran_old_sync COMMON FUNCTION
        WHERE  TRUNC(a.tran_time) < l_etl_date - 2;    --SYSDATE - 3 --#20150905 LocTX change from post time to tran time

        SELECT (CASE
                    WHEN EXISTS(SELECT 1 FROM sync_bk_account_history a WHERE TRUNC(a.tran_time) = l_etl_date )
                    THEN 1
                    ELSE 0
                END)
        INTO l_count
        ...
        IF l_count = 0 THEN
            ...
            AND    TRUNC(tran_time) = l_etl_date;--; TRUNC(SYSDATE - 1);
            ...
            WHERE  sync_type IN (1, 2, 3, 5, 6, 7, 8)
            AND    TRUNC(tran_time) = l_etl_date);--TRUNC(SYSDATE - 1); --1:dd transaction
...
----------------------------------------------------------------------------------------------------*/
   ...
   BEGIN
        ...
        LOOP
            ...
            INSERT INTO sync_lnhist
              SELECT
               ...
               a.lhosbl,
               a.lhdudt
              FROM   hstaging.si_his_lnhist@HSTG_PROD_MSB_READ a,sync_account_info x
              ...
              AND   x.acct_no = LPAD (a.lhacct, 14, '0')--HAONS20150120
        ...
        EXECUTE IMMEDIATE 'alter session close database link staging_pro';
          ...
          BEGIN
            ...
            INSERT INTO sync_cdhist
              SELECT
               ...
               TRIM(a.chuser),
               a.chtime
              FROM   hstaging.si_his_cdhist@HSTG_PROD_MSB_READ a,sync_account_info b
                    ...
                    (SELECT   'FD'
                      ...
                      WHERE       TRIM (a.chtran) = d.tran_service_code(+)
                              ...
                              AND TRUNC (TO_DATE (a.chpstd, 'yyyyddd')) =
                                     TRUNC(TO_DATE (c.host_real_date(+),
                                                    'yyyyddd'))
                              AND x.acct_no = LPAD (a.chacct, 14, '0')--HAONS20150120
                                            AND a.chtime >= g_min_time
                                            AND a.chtime < g_max_time);
            ...
            INSERT INTO twtb_account_history (core_sn, --20160301 QuanPD dung bang tam roi moi day vao bang chinh
                ...
                (SELECT 'FD'|| TO_CHAR (a.chpstd)
                  ...
                  WHERE       TRIM (a.chtran) = d.tran_service_code(+)
                        ...
                        AND info.acct_no = LPAD (a.chacct, 14, '0')
                        AND a.chtime >= g_min_time
                        AND a.chtime < g_max_time);
            ...
            INSERT INTO twtb_account_history (core_sn, --20160301 QuanPD dung bang tam roi moi day vao bang chinh
                ...
                (SELECT
                  ...
                  WHERE       TRIM (a.auxtrc) = d.tran_service_code(+)
                        ...
                        AND info.acct_no = LPAD (a.tracct, 14, '0')
            ...
            INSERT INTO twtb_account_history (core_sn, --20160301 QuanPD dung bang tam roi moi day vao bang chinh
                    ...
                    FROM    sync_etl_lnhist a,
                        ...
                        WHERE TRIM (a.lhtran) = d.tran_service_code(+)
                            ...
                            AND info.acct_no = LPAD (a.lhacct, 14, '0')
                            AND a.lhtime >= g_min_time
                            AND a.lhtime < g_max_time);
      ...
      BEGIN
        ...
        INSERT INTO sync_ddhist
          SELECT
           ...
           TRIM(a.truser),
           TRIM(a.auxtrc)
          FROM   hstaging.si_his_ddhist@HSTG_PROD_MSB_READ a,sync_account_info x
          WHERE  a.dorc IN ('D','C')
          AND    a.trancd NOT IN (77,129,178,185)
          AND    a.trdate >= v_min_date
          AND    a.trdate < (v_checkpoint_date)
          AND  x.acct_no = LPAD(a.tracct ,14,'0')--HAONS20150120
          AND    a.trtime >= g_min_time
          AND    a.trtime < g_max_time;
...
----------------------------------------------------------------------------------------------------*/
    ...
    BEGIN
        ...
        pr_benifit_batch_txn(p_etl_date);
...
*/
  ...
  END cspkg_transaction_etl_sync_dev;
